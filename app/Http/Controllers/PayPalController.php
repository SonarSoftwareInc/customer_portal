...
class PayPalController extends Controller
{
    ...
    public function generateApprovalLink($amountToPay)
    {
        ...
        $transaction->setAmount($amount)
            ->setDescription(
                utrans('billing.paymentToCompany', ['company_name' => config('customer_portal.company_name')])
            )
            /*
             * This is not a payment on a specific invoice, so we'll just generate a unique string, which is what PayPal
             * requires.
             */
        ...
        $redirectUrls
            ->setReturnUrl(
                action([PayPalController::class, 'completePayment'], ['temporary_token' => $tempToken->token])
            )
            ->setCancelUrl(
                action([PayPalController::class, 'cancelPayment'], ['temporary_token' => $tempToken->token])
            );
     ...
     */
    public function completePayment(Request $request, $temporaryToken): Factory|View
    {
        $token = PaypalTemporaryToken::where('token', '=', $temporaryToken)
        ...
        try {
            ...
            $accountBillingController->storePayPalPayment(
                get_user()->account_id,
                $transaction->related_resources[0]->sale->amount->total,
                $transaction->related_resources[0]->sale->id
            );
        } catch (Exception $e) {
     ...
     */
    public function cancelPayment($temporaryToken): Factory|View
    {
        $token = PaypalTemporaryToken::where('token', '=', $temporaryToken)
            ->where('account_id', '=', get_user()->account_id)
            ->first();
